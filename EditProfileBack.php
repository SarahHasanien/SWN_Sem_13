...
{
    //so the logged in user is employee
    $result=$db->conn->query('Select password FROM employee WHERE EID ='.$_SESSION['user2']['EID']);
    $row=$result->fetch_assoc();
    if( $awelEsm == "" || $a5erEsm == "" || $elEmail == "" || $elPwEladeema == "" || $elPwElgedida == "" || $ta2keedElPw == "" || $elkesm == "" || $elmadina == "" || $elbalad == ""  || $elshehadat == "")
    {
        echo '<div class="alert alert-danger" role="alert">
    ...
    if (!empty($_SESSION['user']))
{
     $result=$db->conn->query('Select password FROM manager WHERE MID ='.$_SESSION['user']['MID']);
    $row=$result->fetch_assoc();
    if( $awelEsm == "" || $a5erEsm == "" || $elEmail == "" || $elPwEladeema == "" || $elPwElgedida == "" || $ta2keedElPw == "" || $elkesm == "" || $elmadina == "" || $elbalad == ""  || $elshehadat == "")
    {
        echo '<div class="alert alert-danger" role="alert">
    ...
    echo '<a href="form.php"><button type="button" class="btn btn-primary">Home</span></button></a>';
}
//header("Location:ProfileFront.php");
