...
if (!empty($_SESSION['user2']))
{
    ...
    $d11=0;
    $d12=0;
    $result=$db->conn->query('select rating, year,EID,Month from achievement where EID='.$_SESSION['user2']['EID'].' and year=2017 order by Month ASC');
   ...
   //2016
    ...
    $d11=0;
    $d12=0;
    $result=$db->conn->query('select rating, year,EID,Month from achievement where EID='.$_SESSION['user2']['EID'].' and year=2016 order by Month ASC');
    if ($result->num_rows>0)
        while ($row=$result->fetch_assoc())
        {
            if ($row['Month']=="January")
